...
This package contains all components and assets necessary to use these components within a Drupal theme.

Add the following to the `"repositories"` section your theme's `composer.json` file:
...
This will install the `pds-drupal` package as its own subdirectory within the theme.
...
This will keep your theme directory clean from unnecessary Drupal core files when running Composer commands from the theme directory.
...
This is optional, but will eliminate the need to set these variable each time you use a component.
...
This variable will be used in conjunction with the `block.html.twig` template to programatically set block class names for custom block types.
